/**
 * Manejador mejorado para el filtrado de grados compartidos entre categorías
 ...
 * 
 * Versión con correcciones para evitar que el selector se bloquee.
 */

document.addEventListener('DOMContentLoaded', function() {
    console.log('Inicializando manejador mejorado de grados compartidos...');
    ...
    // Variable para controlar si la validación está activa
    ...
    setTimeout(() => {
        ...
        // Agregar botón de emergencia para restablecer el selector de grados
        agregarBotonEmergencia();

        // Verificar inmediatamente si el selector de grados necesita desbloquearse
        const gradosSelect = document.getElementById('grado');
        if (gradosSelect && gradosSelect.disabled) {
            console.log('Selector de grados bloqueado. Desbloqueando...');
            gradosSelect.disabled = false;
            mostrarMensaje('Selector de grados desbloqueado automáticamente', 'info');
        }
    }, 1000);
    ...
    function cargarGradosCategorias() {
        ...
        if (!csrfToken || !csrfToken.content) {
            ...
            // Asegurarse de que el selector de grados sea utilizable
            restaurarSelectGradosOriginal();
        ...
        const gradosSelect = document.getElementById('grado');
        if (gradosSelect) {
            // Guardar el valor actual en caso de que necesitemos restaurarlo
            const valorActual = gradosSelect.value;
            
            // En lugar de deshabilitar el selector, que podría causar problemas,
            // solo mostramos un mensaje de carga
            mostrarMensaje('Cargando grados por categoría...', 'info');
        ...
        .then(data => {
            ...
            // Habilitar el selector de grados pase lo que pase
            if (gradosSelect) {
                gradosSelect.disabled = false;
            }
        ...
        .catch(error => {
            ...
            mostrarMensaje('Error al cargar los grados por categoría. Se ha desactivado la validación.', 'error');
            
            // Asegurarse de que el selector de grados sea utilizable
            restaurarSelectGradosOriginal();
            
            // Mostrar el botón de emergencia inmediatamente
            const botonEmergencia = document.getElementById('btn-restablecer-grados');
            if (botonEmergencia) {
    ...
    /**
     * Restaura el selector de grados a su estado original
     */
    function restaurarSelectGradosOriginal() {
        const gradosSelect = document.getElementById('grado');
        if (!gradosSelect) return;
        
        // Guardar el valor actualmente seleccionado para intentar restaurarlo después
        const valorSeleccionado = gradosSelect.value;
        
        // Limpiar clases de estilo que puedan afectar
        gradosSelect.classList.remove('tiene-grados-compartidos', 'sin-grados-compartidos');
        gradosSelect.classList.add('grado-restaurado');
        
        // Forzar a habilitar el selector
        gradosSelect.disabled = false;
        
        // Limpiar todas las opciones actuales excepto la primera (placeholder)
        while (gradosSelect.options.length > 1) {
            gradosSelect.remove(1);
        }
        
        // Recuperar las opciones originales del HTML
        const grados = [
        ...
        if (valorSeleccionado && gradosSelect.querySelector(`option[value="${valorSeleccionado}"]`)) {
            gradosSelect.value = valorSeleccionado;
        } else {
            gradosSelect.selectedIndex = 0;
        }
        ...
        // Añadir una etiqueta de información sobre el restablecimiento
        const nuevaEtiqueta = document.createElement('span');
        nuevaEtiqueta.className = 'grado-info restaurado';
        nuevaEtiqueta.textContent = 'Selector restablecido - Todos los grados disponibles';
        nuevaEtiqueta.style.color = '#28a745';
        
        if (gradosSelect.parentNode) {
            gradosSelect.parentNode.appendChild(nuevaEtiqueta);
            
            // Auto-eliminar la etiqueta después de unos segundos
            setTimeout(() => {
                if (nuevaEtiqueta.parentNode) {
                    nuevaEtiqueta.remove();
                }
            }, 5000);
        }
        
        // Ocultar el botón de emergencia ya que ya no es necesario
        const botonEmergencia = document.getElementById('btn-restablecer-grados');
        if (botonEmergencia) {
        ...
        console.log('Selector de grados restaurado a su estado original');
    }
    ...
    function mostrarMensaje(mensaje, tipo = 'info') {
        // Eliminar mensajes anteriores con el mismo texto para evitar duplicados
        document.querySelectorAll('.mensaje-temporal').forEach(el => {
            if (el.textContent.trim() === mensaje.trim() && el.parentNode) {
                el.remove();
            }
        });
        ...
        mensajeElement.style.maxWidth = '400px';
        mensajeElement.style.boxShadow = '0 4px 8px rgba(0,0,0,0.2)';
        mensajeElement.style.fontSize = '0.95rem';
        ...
        // Duración del mensaje según su tipo
        const duracion = tipo === 'error' || tipo === 'warning' ? 8000 : 5000;
    ...
    function actualizarCategoriasSeleccionadas() {
        ...
        // Asegurarse de que el selector de grados esté habilitado antes de actualizarlo
        const gradosSelect = document.getElementById('grado');
        if (gradosSelect) {
            gradosSelect.disabled = false;
        }
    ...
    function actualizarDesplegableGrados() {
        ...
        // Asegurar que el selector esté siempre habilitado
        gradosSelect.disabled = false;
        ...
        if (categoriasSeleccionadas.length === 0) {
            ...
            // Asegurarse de que haya opciones en el selector
            if (gradosSelect.options.length <= 1) {
                restaurarSelectGradosOriginal();
            }
        ...
        if (Object.keys(gradosPorCategoria).length === 0) {
            ...
            // Si no hay datos, asegurar que el selector esté utilizable
            restaurarSelectGradosOriginal();
            return;
        }
        ...
        if (categoriasSeleccionadas.length === 1 && gradosPorCategoria[categoriasSeleccionadas[0]]) {
            ...
            // Si no hay grados para esta categoría, mostrar un mensaje y restaurar todos
            if (gradosCategoria.length === 0) {
                mostrarMensaje('La categoría seleccionada no tiene grados asociados. Se muestran todos los grados disponibles.', 'warning');
                restaurarSelectGradosOriginal();
                return;
            }
        ...
        // Si no hay grados comunes, pero hay categorías seleccionadas
        if (gradosComunes.length === 0 && categoriasSeleccionadas.length > 0) {
            // Mostrar mensaje de alerta
            mostrarMensaje("No hay grados compatibles entre las categorías seleccionadas. Se muestran todos los grados disponibles.", "warning");
            
            // Restaurar todas las opciones originales para permitir selección
            opcionesOriginales.forEach(opcion => {
                // Omitir la primera opción de "Seleccione un grado"
                if (opcion.value === "") return;
                
                // Añadir todas las opciones pero marcar como no compatibles
                const nuevaOpcion = opcion.cloneNode(true);
                nuevaOpcion.classList.add('grado-no-compatible');
                nuevaOpcion.style.color = "#777"; // Color gris para indicar advertencia
                gradosSelect.appendChild(nuevaOpcion);
            });
            ...
            // Agregar etiqueta de información
            mostrarEtiquetaInformacion(gradosSelect, false, "Sin grados compartidos (selección manual)");
            
            // Mantener el valor actual si tenía uno
            if (valorActual) {
                gradosSelect.value = valorActual;
            } else {
                gradosSelect.value = "";
    ...
    /**
     * Agrega un botón de emergencia para restablecer el selector de grados
     */
    function agregarBotonEmergencia() {
        const gradosSelect = document.getElementById('grado');
        if (!gradosSelect || !gradosSelect.parentNode) return;
        
        // Verificar si ya existe el botón
        if (document.getElementById('btn-restablecer-grados')) return;
        
        // Crear el botón de emergencia
        const botonEmergencia = document.createElement('button');
        botonEmergencia.type = 'button';
        botonEmergencia.id = 'btn-restablecer-grados';
        botonEmergencia.className = 'btn btn-sm btn-danger mt-1';
        botonEmergencia.style.display = 'none'; // Oculto por defecto
        botonEmergencia.style.fontSize = '0.8rem';
        botonEmergencia.style.width = '100%';
        botonEmergencia.innerHTML = '<i class="fas fa-sync-alt me-1"></i> Restablecer selector de grados';
        
        // Configurar evento de clic
        botonEmergencia.addEventListener('click', function(e) {
            e.preventDefault();
            // Habilitar primero el selector
            if (gradosSelect) gradosSelect.disabled = false;
            // Luego restaurar las opciones
            restaurarSelectGradosOriginal();
            validacionActiva = false; // Desactivar validación
            mostrarMensaje('Selector de grados restablecido. Se ha desactivado la validación de grados compartidos.', 'success');
        ...
        // Mostrar el botón inmediatamente si el selector está deshabilitado
        if (gradosSelect.disabled) {
            botonEmergencia.style.display = 'block';
            mostrarMensaje('El selector de grados está bloqueado. Usa el botón "Restablecer selector de grados" para desbloquearlo.', 'warning');
        }
        
        // Mostrar el botón si ocurre un error en el selector
        gradosSelect.addEventListener('focus', function() {
            // Si el selector está vacío o deshabilitado, mostrar el botón de emergencia
            if (gradosSelect.options.length <= 1 || gradosSelect.disabled) {
                botonEmergencia.style.display = 'block';
            }
        });
        
        // También mostrar el botón después de unos segundos si el selector está vacío o deshabilitado
        setTimeout(() => {
            if (gradosSelect.options.length <= 1 || gradosSelect.disabled) {
                botonEmergencia.style.display = 'block';
                mostrarMensaje('Si tienes problemas con el selector de grados, puedes restablecerlo con el botón debajo', 'info');
            }
        }, 2000);
    }
    ...
    // Agregar un temporizador para verificar periódicamente si el selector está bloqueado
    setInterval(() => {
        const gradosSelect = document.getElementById('grado');
        if (gradosSelect && gradosSelect.disabled) {
            console.log('Selector de grados bloqueado detectado. Desbloqueando automáticamente...');
            gradosSelect.disabled = false;
            
            // Verificar si hay necesidad de restaurar opciones
            if (gradosSelect.options.length <= 1) {
                restaurarSelectGradosOriginal();
            }
